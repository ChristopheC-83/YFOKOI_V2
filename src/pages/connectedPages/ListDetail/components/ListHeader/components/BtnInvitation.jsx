...
export default function BtnInvitation({ title, shareCode }) {
  const [copied, setCopied] = useState(false);

  const code = shareCode || "NON GÉNÉRÉ";
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="p-3 bg-card border border-border rounded-2xl shadow-sm hover:bg-primary/10 transition-colors relative cursor-pointer text-clip-2">
          <FiShare2 size={24} />
        </button>
      </DialogTrigger>
